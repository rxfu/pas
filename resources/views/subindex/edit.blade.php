...
<form method="post" action="{{ route('subindex.update', $subindex->id) }}">
	...
	<div class="form-group row">
		...
		<div class="col-md-6 col-sm-6 col-xs-12">
			<textarea class="form-control col-md-7 col-xs-12" rows="5" id="description" name="description" placeholder="观测点">{{ $subindex->description }}</textarea>
		</div>
	</div>
	<div class="form-group row">
	...
	<div class="form-group row">
		...
		<div class="col-md-6 col-sm-6 col-xs-12">
			<div class="form-check form-check-inline">
				<input type="radio" class="form-check-input" id="is_manager_yes" name="is_manager" value="1"{{ $subindex->is_manager ? ' checked' : '' }}>
				<label for="is_manager_yes" class="form-check-label">是</label>
			</div>
			<div class="form-check form-check-inline">
				<input type="radio" class="form-check-input" id="is_manager_no" name="is_manager" value="0"{{ $subindex->is_manager ? '' : ' checked' }}>
